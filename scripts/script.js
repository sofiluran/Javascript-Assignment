...
if (suspectInterview === "Elsa") {
  ...
  if (interviewTechnique === 1) {
    alert(`You grab your gun from your holster and point it at ${suspectInterview}. "Did you do it!? Did you kill the conductor!?" ${suspectInterview} starts crying. "No i didn't!" You realize this wasn't the best method to get an answer... It's time to leave crying ${suspectInterview} alone and conduct another interview...`)
  } else {
    alert(`You whisper softly in ${suspectInterview}s ear: "What happened". ${suspectInterview} quietly answers: I'm not sure, everything happened so fast. Maybe interview someone else detective ${detective}...`)
  }
} else if (suspectInterview === "Bobby") {
  interviewTechnique = parseInt(prompt(`And how do you want to interview ${suspectInterview}? 1, By holding ${suspectInterview} at gunpoint or 2, by whispering in ${suspectInterview}s ear? 
  Enter 1 or 2.`))
  if (interviewTechnique === 1) {
    alert(`You grab your gun from your holster and point it at ${suspectInterview}. "Did you do it!? Did you kill the conductor!?" ${suspectInterview} starts crying. "No i didn't!" You realize this wasn't the best method to get an answer... It's time to leave crying ${suspectInterview} alone and conduct another interview.`)
...
} else if (suspectInterview === "Skipper") {
  interviewTechnique = parseInt(prompt(`And how do you want to interview ${suspectInterview}? 1, By holding ${suspectInterview} at gunpoint or 2, by whispering in ${suspectInterview}s ear? 
  Enter 1 or 2.`))
  if (interviewTechnique === 1) {
    alert(`You grab your gun from your holster and point it at ${suspectInterview}. "Did you do it!? Did you kill the conductor!?" ${suspectInterview} starts crying. "No i didn't!" You realize this wasn't the best method to get an answer... It's time to leave crying ${suspectInterview} alone and conduct another interview.`)
...
} else if (suspectInterview === "Viola") {
  interviewTechnique = parseInt(prompt(`And how do you want to interview ${suspectInterview}? 1, By holding ${suspectInterview} at gunpoint or 2, by whispering in ${suspectInterview}s ear? 
  Enter 1 or 2.`))
  if (interviewTechnique === 1) {
    alert(`You grab your gun from your holster and point it at ${suspectInterview}. "Did you do it!? Did you kill the conductor!?" ${suspectInterview} starts crying. "No i didn't!" You realize this wasn't the best method to get an answer... It's time to leave crying ${suspectInterview} alone and conduct another interview.`)
...
let remainingSuspects = SUSPECTS.filter(suspect => suspect !== suspectInterview)

let nextSuspect = prompt(`You interviewed ${suspectInterview}, without any luck. Who do you want to interview next? ${remainingSuspects}`)
...
while (suspectLocation !== 1) {
  ...
  if (suspectLocation === 1) {
    alert(`As you're walking on to the stage you suddenly feel the floor beneth you dissapear! WAAAAAAAAHHHHHHHhhhhhhhh!
...
alert(`You got the right key in ${counter} guesses! Now lets continue the murder mystery!`)  // UNDEFINED varför? 


let lastRemainingSuspects = SUSPECTS.filter(suspect => suspect !== suspectInterview && suspect !== nextSuspect)

lastRemainingSuspects = prompt(`Who do you want to interview next? Choose between ${lastRemainingSuspects}.`)

if (lastRemainingSuspects === lastRemainingSuspects) {
  alert(`Good chice detevtive ${detective}! You found ${lastRemainingSuspects} sitting calmly on a chair in the corner of the stage. "So ${lastRemainingSuspects}, tell me what you saw tonight?" But before ${lastRemainingSuspects} answered you found something laying under the chair. When you look closer you see that it's a written handnote saying: "You will pay for what you did..."`)
}

for (let i = 0; i < 2; i++) {
  let chooseClue = parseInt(prompt(`1 handwriting, 2 marks on the floor`))
...
// Kanske göra om till loop så man kan gissa igen? 
...
// hitta en ledtråd...
// rulla en tärning, 8 ball.
// who do you want to question next? 
// you cant decide, you grab something from your inner jack pocket. 
// Its your magic 8 ball. Enter a number to get help from the magic 8 ball. 
